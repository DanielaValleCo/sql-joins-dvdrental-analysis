...
Transformar preguntas de negocio en consultas SQL y generar insights a partir de una base relacional compuesta por múltiples tablas.
...
Se encontraron tres películas dentro del catálogo que no tienen actores registrados en la tabla `film_actor`.
...
Se utilizó un `LEFT JOIN` entre `film` y `film_actor` para conservar todas las películas del catálogo, incluyendo aquellas que no tenían coincidencias en la tabla de actores. Posteriormente, se filtraron los registros donde `fa.actor_id IS NULL`, lo que permitió identificar películas sin actores asociados.
